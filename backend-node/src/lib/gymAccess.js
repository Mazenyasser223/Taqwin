...
const MEMBER_USER_SELECT = {
  ...
  email: true,
  phone: true,
  profile: { select: { displayName: true, avatarUrl: true, gender: true, onboardingData: true } },
};
...
function formatMemberRow(membership, openVisit, now = new Date()) {
  ...
  return {
    ...
    gender: normalizeGender(membership.user?.profile?.gender),
    address: extractMemberAddress(membership.user?.profile),
  };
}
