const TEXT_NEGATIVE_PATTERN =
  /\b(bad|terrible|awful|worst|dirty|broken|old|disappoint|hate|never|avoid|overpriced|crowded|rude|unprofessional|waste)\b|سيئ(?:ة|ه)?|قديم(?:ة|ه)?|وسخ(?:ة|ه)?|ردي(?:ء|)|مخيب|مزعج|فظ|غالي|زحمة|لا\s*أنصح|اسوأ|أسوأ|وحش|مش\s*نظيف|مو\s*نظيف|غير\s*نظيف|بقذر|قذر|مزري/i;
...
const TEXT_NEGATED_POSITIVE_PATTERN = /مش\s*نظيف|مو\s*نظيف|غير\s*نظيف|not\s+clean/i;
...
function textSentimentLabel(body: string): 'positive' | 'negative' | 'neutral' | null {
  ...
  if (!text) return null;
  const negatedPositive = TEXT_NEGATED_POSITIVE_PATTERN.test(text);
  const negative = TEXT_NEGATIVE_PATTERN.test(text) || negatedPositive;
  const positive = TEXT_POSITIVE_PATTERN.test(text) && !negatedPositive;
