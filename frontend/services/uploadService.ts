...
class UploadService {
  ...
  async uploadFile(file: File, folder: UploadFolder): Promise<{ url?: string; error?: string }> {
    ...
    // Local upload first — avoids Supabase CORS / bucket 404 in dev
    const local = await this.uploadFileLocal(file, folder);
    if (local.url) return local;
    if (local.error && !local.error.includes('401') && !local.error.includes('403')) {
      // If local failed for a reason other than auth, still try Supabase below
    }
    ...
    try {
      ...
      if (!res.ok) {
        return { error: local.error || `Upload failed (${res.status})` };
      }
      return { url: sign.data.publicUrl };
    } catch {
      return { error: local.error || 'Network error during upload' };
    }
  }
