    ...
    <div className="space-y-12">
      <div className="flex justify-between items-center relative">
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={snapTransition} className="relative z-10">
          ...
          <p className="text-slate-400 mt-2">Gear and supplements verified for the Taqwin community.</p>
        </motion.div>
      ...
      {loading && <div className="text-primary animate-pulse">Loading products…</div>}
      {error && <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-sm">{error}</div>}
      {!loading && !error && products.length === 0 && (
        <div className="glass-panel p-10 rounded-3xl text-center text-slate-400">No products yet.</div>
      )}
            ...
            <motion.div variants={itemVariants} className="bg-surface border border-border rounded-[2.5rem] overflow-hidden group hover:border-primary transition-all flex flex-col h-full shadow-2xl">
              ...
              <div className="p-8 flex flex-col flex-1 gap-6">
                <div className="space-y-1">
                  ...
                  {product.description && <p className="text-sm text-slate-400 line-clamp-2">{product.description}</p>}
                </div>
            ...
            >
              ...
              {cart.items.length === 0 ? (
                ...
                <div className="space-y-3 overflow-y-auto custom-scrollbar flex-1">
                  {cart.items.map((item) => (
                    <div key={item.product.id} className="flex items-center gap-3 p-3 rounded-2xl bg-white/5 border border-white/5">
                      ...
                      <input
                        ...
                        className="w-16 bg-white/5 border border-white/10 rounded-lg px-2 py-1 text-center font-bold"
                      />
                      <button onClick={() => cart.remove(item.product.id)} className="text-slate-400 hover:text-red-400">
