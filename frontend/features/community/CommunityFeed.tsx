    ...
    <div className="max-w-3xl mx-auto space-y-10">
      <div className="flex justify-between items-end relative">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={snapTransition} className="relative z-10">
          ...
          <p className="text-slate-400 mt-2 font-medium">Share progress, comment, and stay connected.</p>
        </motion.div>
      ...
      {loading && <div className="text-primary animate-pulse">Loading feed…</div>}
      {error && <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-sm">{error}</div>}
      {!loading && posts.length === 0 && (
        <div className="glass-panel p-10 rounded-3xl text-center text-slate-400">
          Nothing posted yet. Be the first to share progress!
        </div>
      )}
              ...
              <div className="p-8 flex items-center justify-between">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em]">{timeAgo(post.createdAt)}</p>
                  </div>
                </div>
                {isMine && (
                  <button onClick={() => handleDelete(post.id)} className="text-slate-500 hover:text-red-400 transition-colors">
              ...
              <div className="p-8 border-t border-border flex items-center justify-between">
                <div className="flex items-center gap-10">
                  <button
                    ...
                    className={`flex items-center gap-3 transition-colors group ${post.likedByMe ? 'text-primary' : 'text-slate-400 hover:text-primary'}`}
                  >
                    <motion.span
                  ...
                  <button onClick={() => loadComments(post.id)} className="flex items-center gap-3 text-slate-400 hover:text-primary transition-colors group">
                    ...
                    <div className="p-6 space-y-4">
                      ...
                      {comments.map((c) => (
                        <div key={c.id} className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-slate-500 mt-1">{timeAgo(c.createdAt)}</p>
                          </div>
                        </div>
                      ))}
                      <div className="flex gap-2 pt-2 border-t border-white/5">
                        <input
                          ...
                          className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40"
                        />
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setComposerOpen(false)} className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center p-6">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} onClick={(e) => e.stopPropagation()} className="glass-panel w-full max-w-lg rounded-3xl p-8 space-y-6">
              ...
              <textarea
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary/40 font-medium"
              />
              <input
                value={composerImage}
              ...
              <div className="flex gap-3">
                <button onClick={() => setComposerOpen(false)} className="flex-1 bg-white/5 border border-white/10 py-3 rounded-xl font-bold">Cancel</button>
