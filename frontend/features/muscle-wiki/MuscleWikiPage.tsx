...
export function MuscleWikiPage() {
  ...
  return (
    <div className="min-h-screen bg-[#070b12]">
      <div aria-hidden className="pointer-events-none fixed inset-0 bg-[radial-gradient(ellipse_at_top,_rgba(34,211,238,0.08),_transparent_50%),radial-gradient(ellipse_at_bottom_right,_rgba(244,63,94,0.06),_transparent_45%)]" />
      <div className="relative mx-auto flex min-h-screen max-w-7xl flex-col gap-6 p-4 md:gap-8 md:p-8 lg:flex-row lg:gap-10">
        <header className="lg:hidden">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-400/80">Taqwin Fitness</p>
          <h1 className="mt-1 text-2xl font-bold text-white">Captain Hema Muscle Wiki</h1>
        </header>
        ...
        <section className="flex min-h-0 flex-1 flex-col lg:min-h-[calc(100vh-4rem)] lg:w-[58%]">
          <div className="mb-4 hidden shrink-0 lg:block">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-400/80">Interactive 3D</p>
            <h1 className="mt-1 text-3xl font-bold text-white">Captain Hema Muscle Wiki</h1>
            <p className="mt-2 max-w-md text-sm text-slate-400">
              Rotate the model and tap a muscle zone to reveal curated exercises.
            </p>
          </div>
